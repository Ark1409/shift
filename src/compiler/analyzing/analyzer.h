        ...
        struct scope {
            ...
            utils::ordered_set<shift_variable*> find_variables(const std::string_view name) const {
                ...
                if ((func && (!parent || parent->func != func)) || (var && (!parent || parent->var != var))) {
                    if ((var && (var->type.shift_mods & shift_mods::STATIC) == 0x0 && var->clazz)) {
                        if (name == "this") {
                    ...
                    } else if ((func && (func->shift_mods & shift_mods::STATIC) == 0x0 && func->clazz)) {
                        if (name == "this") {
            ...
            inline std::strong_ordering operator<=>(const match_quality& other) const noexcept {
                ...
                if (func && other.func) {
                    ...
                    for (auto params_it = func->parameters.begin(), other_params_it = other.func->parameters.begin();
                        ...
                        auto current_imut = (params_it->second.type.shift_mods & shift_mods::IMUT);
                        auto other_imut = (other_params_it->second.type.shift_mods & shift_mods::IMUT);
