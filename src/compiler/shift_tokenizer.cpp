...
namespace shift::compiler {
    SHIFT_API void tokenizer::rollback() noexcept {
    ...
    SHIFT_API tokenizer::const_iterator tokenizer::position_at(const file_indexer index) const noexcept {
    ...
    SHIFT_API tokenizer::const_iterator tokenizer::position_before(const file_indexer index) const noexcept {
        auto it = std::lower_bound(this->m_tokens.cbegin(), this->m_tokens.cend(), index, [](const token& token, const file_indexer index) {
    ...
    SHIFT_API tokenizer::const_iterator tokenizer::position_after(const file_indexer index) const noexcept {
    ...
    SHIFT_API const token& tokenizer::peek_token(size_type count) const noexcept {
        count = std::min<size_type>(count, this->end() - this->m_token_index);
        return this->token_at(this->m_token_index + count);
    }

    SHIFT_API const token& tokenizer::reverse_token(size_type count) noexcept {
        count = std::min<size_type>(count, this->m_token_index - this->begin());
        return this->token_at(this->m_token_index -= count);
    }

    SHIFT_API const token& tokenizer::reverse_peek_token(size_type count) const noexcept {
        if (count > size_type(this->m_token_index - this->begin())) return token::null;
        return this->token_at(this->m_token_index - count);
    }

    SHIFT_API const token& tokenizer::next_token(size_type count) noexcept {
        count = std::min<size_type>(count, this->end() - this->m_token_index);
        return this->token_at(this->m_token_index += count);
    }

    SHIFT_API void tokenizer::tokenize() {
