...
namespace shift::compiler {
	SHIFT_API void tokenizer::rollback(void) noexcept {
	...
	SHIFT_API typename std::vector<token>::const_iterator tokenizer::set_index(const typename std::vector<token>::const_iterator index) noexcept {
	...
	SHIFT_API const token& tokenizer::reverse_token(typename std::vector<token>::size_type count) noexcept {
		count = std::min<typename std::vector<token>::size_type>(count, this->m_token_index - this->cbegin());
		return this->token_at(this->m_token_index -= count);
	}

	SHIFT_API const token& tokenizer::reverse_peek_token(typename std::vector<token>::size_type count) const noexcept {
		if (count > typename std::vector<token>::size_type(this->m_token_index - this->cbegin())) return token::null;
		return this->token_at(this->m_token_index - count);
	}

	SHIFT_API const token& tokenizer::next_token(typename std::vector<token>::size_type count) noexcept {
		count = std::min<typename std::vector<token>::size_type>(count, this->cend() - this->m_token_index);
		return this->token_at(this->m_token_index += count);
	}
	...
	SHIFT_API void tokenizer::tokenize(void) {
		...
		{ // tokenizing
			...
			this->m_lines.reserve(utils::count((std::string_view)this->m_filedata, "\n"sv) + 1);
