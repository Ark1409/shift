...
namespace shift::utils {
    ...
    NumT str_to_num(const std::string& str) {
        ...
        if constexpr (std::is_floating_point_v<NumT>) {
            ...
            if constexpr (!std::is_signed_v<NumT>) {
                ...
                if (temp > std::numeric_limits<NumT>::max() || temp < std::numeric_limits<NumT>::min())
                    throw std::runtime_error("shift::utils::str_to_num");
                ret = static_cast<NumT>(temp);
            }
        }
    ...
    template<typename T>
    inline std::stack<T>&
        pop_stack(std::stack<T>& stack, typename std::stack<T>::size_type count = typename std::stack<T>::size_type(1)) {
    ...
    template<typename T>
    std::list<T> sub_list(const std::list<T>& list, const typename std::list<T>::size_type index,
        const typename std::list<T>::size_type length) {
        ...
        if (length < 0 || (index + length) >= list.size()) {
            throw std::out_of_range(
                std::to_string(index + length) + " is not in range [0, " + std::to_string(list.size()) + ")");
        }

        std::list<T> sub;
        typename std::list<T>::size_type i = 0;
        for (typename std::list<T>::const_iterator it = list.cbegin();
            it != list.cend() && i < (index + length); it++, i++) {
            if (i >= index) {
                sub.push_back(*it);
            }
        }
    ...
    template<typename T>
    std::list<T> sub_list(std::list<T>&& list, const typename std::list<T>::size_type index,
        const typename std::list<T>::size_type length) {
        using namespace std::string_literals;
        if (index < 0 || index >= list.size())
            throw std::out_of_range(std::to_string(index) + " is not in range [0," + std::to_string(list.size()) + ")");
    ...
    template<typename T>
    std::vector<T> sub_vector(const std::vector<T>& vec, const typename std::vector<T>::size_type index,
        ...
        if (length < 0 || (index + length) >= vec.size()) {
            throw std::out_of_range(
                std::to_string(index + length) + " is not in range [0, " + std::to_string(vec.size()) + ")");
        }
    ...
    template<typename T>
    std::vector<T> sub_vector(std::vector<T>&& vec, const typename std::vector<T>::size_type index,
        const typename std::vector<T>::size_type length) {
        using namespace std::string_literals;
        if (index < 0 || index >= vec.size())
            throw std::out_of_range(std::to_string(index) + " is not in range [0," + std::to_string(vec.size()) + ")");
    ...
    std::basic_string<CharT, Traits, Alloc> repeat(const std::basic_string<CharT, Traits, Alloc>& str, const std::size_t count) {
    ...
    std::basic_string<CharT, Traits, Alloc> repeat(const std::basic_string_view<CharT, Traits> str, const std::size_t count) {
    ...
    std::vector<std::basic_string_view<CharT, Traits>> split(const std::basic_string_view<CharT, Traits> str,
        ...
        size_type last = 0;
        if (delim_len > 0) {
            for (size_type begin = str.find(delim); begin !=
                std::basic_string_view<CharT, Traits>::npos; begin += delim_len, last = begin, begin = str.find(
                    delim, begin)) {
    ...
    template<typename... Ts> requires (sizeof...(Ts) > 0 && (std::is_convertible_v<Ts, std::size_t> && ...))
        constexpr inline std::size_t hash_combine(const std::size_t first, const std::size_t second, const Ts... rest) noexcept {
        return hash_combine(hash_combine(first, second), rest...);
    }

    template<class... Ts>
    struct visit_overloader : Ts... {
        constexpr visit_overloader(Ts&&... ts) noexcept((std::is_nothrow_move_constructible_v<Ts> && ...)) : Ts(std::move(ts))... {}
        constexpr visit_overloader(const Ts&... ts) noexcept((std::is_nothrow_copy_constructible_v<Ts> && ...)) : Ts(ts)... {}
...
template<typename CharT, typename Traits, typename T>
std::basic_ostream<CharT, Traits>& operator<<(std::basic_ostream<CharT, Traits>& _os, const std::list<T>& __list) {
    _os << CharT('[');
    bool past_first = false;
    for (T const& _v : __list) {
        if (past_first)
            _os << CharT(',') << CharT(' ');
        _os << _v;
        past_first = true;
...
operator+(typename std::list<T>::iterator it, typename std::list<T>::difference_type count) noexcept;

template<typename T>
inline typename std::list<T>::const_iterator
operator+(typename std::list<T>::const_iterator it, typename std::list<T>::difference_type count) noexcept;
