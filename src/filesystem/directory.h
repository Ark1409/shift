...
namespace shift::filesystem {
    ...
    public:
         ...
         */
        SHIFT_API bool exists() const noexcept;
         ...
         */
        SHIFT_API bool remove() noexcept;

        /**
         * Gets the parent directory of this directory, returning the current directory if it has no parent.
         * @return The parent directory
         */
        inline directory get_parent() const {
            return directory(std::filesystem::absolute(std::filesystem::weakly_canonical(this->m_path)).parent_path());
        }
         ...
         */
        SHIFT_API std::list<directory> get_subdirectories() const;
         ...
         */
        SHIFT_API std::list<file> get_subfiles() const;
         ...
         */
        inline std::filesystem::path& raw_path() noexcept { return this->m_path; }
         ...
         * @return The path linked to this directory, as a std::filesystem::path
         */
        inline const std::filesystem::path& raw_path() const noexcept { return this->m_path; }

        /**
         * Retrieves the name of the current directory.
         * @return The current directory name.
         */
        inline std::string get_name() const { return this->m_path.filename().string(); }

        /**
         * Retrieves the size of all the directory's content's combined (sub-files and sub-directories).
         * @return The size of the directory.
         */
        SHIFT_API std::uintmax_t get_size() const;

        inline std::uintmax_t size() const { return get_size(); }
         ...
         */
        inline std::string get_absolute_path() const { return std::filesystem::absolute(this->m_path).string(); }

        /**
         * Retrieves a std::filesystem::path containing the absolute path of the current directory.
         * @return The absolute path of the current directory.
         */
        inline std::string get_canonical_path() const { return std::filesystem::weakly_canonical(this->m_path).string(); }

        inline directory get_absolute_directory() const { return directory(this->get_absolute_path()); }

        inline directory get_canonical_directory() const { return directory(this->get_canonical_path()); }
         ...
         */
        inline static directory get_current_path() { return directory(std::filesystem::current_path()); }

        inline static directory get_current_directory() { return get_current_path(); }

#ifdef SHIFT_SUBSYSTEM_WINDOWS

        inline drive get_drive() const {
...
operator<<(std::basic_ostream<_CharT, _Traits>& __os, const shift::filesystem::directory& __dir) {
