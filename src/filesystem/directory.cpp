...
namespace shift::filesystem {
	SHIFT_API bool directory::exists(void) const noexcept {
		try {
	...
	SHIFT_API bool directory::remove(void) noexcept {
		try {
			if (!this->exists())
				return true;
	...
	SHIFT_API std::list<directory> directory::get_subdirectories(void) const {
	...
	SHIFT_API std::list<file> directory::get_subfiles(void) const {
		std::list<file> files;
		for (const auto& path : std::filesystem::directory_iterator(this->m_path)) {
	...
	SHIFT_API std::uintmax_t directory::get_size(void) const {
